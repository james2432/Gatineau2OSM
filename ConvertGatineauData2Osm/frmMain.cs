                    ...
                    case "Address Change":
                        ...
                        switch (Type)
                        {
                            case OSMEntityType.node:
                                ...
                                //Update new coodinates
                                int pos = resultset.IndexOf("lat=\"");
                                resultset = resultset.Replace(resultset.Substring(pos, resultset.IndexOf("\" ", pos) - pos+1), "lat=\"" + extract_lat_long(dr["Geom"].ToString(), true)+"\"");
                                pos = resultset.IndexOf("lon=\"");
                                resultset = resultset.Replace(resultset.Substring(pos, resultset.IndexOf("\" ", pos) - pos+1), "lon=\"" + extract_lat_long(dr["Geom"].ToString(), false) + "\"");
                            break;
                            case OSMEntityType.way:
                                resultset = resultset.Replace("<way", "<way action='modify'");
                            break;
